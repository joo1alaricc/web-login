...
function setupBackground() {
    if (window.CONFIG && window.CONFIG.BACKGROUND_IMAGE) {
        document.getElementById('background-body').style.backgroundImage = `url('${window.CONFIG.BACKGROUND_IMAGE}')`;
    }
}

// Setup audio element
function setupAudio() {
    if (window.CONFIG && window.CONFIG.SUCCESS_AUDIO) {
        const audio = document.getElementById('success-audio');
        audio.src = window.CONFIG.SUCCESS_AUDIO;
        audioElement = audio;
        audio.loop = false;
    }
}
...
function loadUserData() {
    ...
    fetch(window.CONFIG.GITHUB_DATA_URL)
        .then(response => {
            if (!response.ok) {
...
function saveUserStatus() {
    ...
    // Jika CONFIG tidak tersedia atau token tidak disediakan, hentikan proses
    if (!window.CONFIG || !window.CONFIG.GITHUB_TOKEN) {
        console.log('CONFIG atau token GitHub tidak disediakan, melewatkan penyimpanan ke GitHub');
        return;
    }
    ...
    fetch(window.CONFIG.GITHUB_STATUS_URL, {
        headers: {
        ...
        .then(data => {
            ...
            return fetch(window.CONFIG.GITHUB_STATUS_URL, {
                method: 'PUT',
                headers: {
                    'Authorization': `token ${window.CONFIG.GITHUB_TOKEN}`,
